...
Goal metrics (G1–G7) come from your own `~/.orchestra/orchestra.db` over a ≥ 30-day window using `orch` queries recorded in `evidence/goals/`, with the method written next to each number — for example G2 as `count(tasks where model.tier < max) / count(tasks)`, G3 as `count(review rounds where reviewer.provider ≠ author.provider) / count(review rounds)`, G6 as the p95 of `ttdMs` and `ttrMs` from `repair_cases`. A number without its query is not evidence.
...
- **Metrics windows.** G2/G3 need ≥ 30 days of real usage on real work; if the dev machine was mostly running demos, the numbers are not evidence. Start the measurement window at the beginning of M10, not at the end.
