...
**MVP = M0 + M1.** Exit: Claude Code and Codex (Antigravity only if the ADR-008 gate is resolved) running live in tmux panes inside per-task git worktrees, visible and controllable from the browser, with supported structured prompts surfaced in Attention with evidenced answer channels and explicit manual-only limitations, plus "Quick Delegate" of a single task to a manually chosen provider. Everything after M1 is layered on top without rewriting the substrate.
