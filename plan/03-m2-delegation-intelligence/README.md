...
| M1-05, M1-06, M1-07 ✅ | M2-03 (manifests) may start as soon as the three adapters and their fixtures exist |
...
- [ ] ≥ 8 model profiles across claude / codex / agy load; a local override in `~/.orchestra/catalog-overrides/` is reflected in the Models screen within 2 s without daemon restart.
- [ ] `manifest.contract.spec.ts` + new `commands-discovery` and `prompt-protocol` contract specs pass for all three providers on the pinned CLI versions.
...
- [ ] Daemon restart during M2 flows loses no decision, task, or open `ask_user` prompt.
...
| M2-03 | [step-03-capability-manifests-v1-full.md](step-03-capability-manifests-v1-full.md) | Capability manifests v1 (full) | 2.5 d | M1-05, M1-06, M1-07 ∥ |
...
2. Open **Models** → filter provider `claude` → open `claude/opus` → radar shows 6 dimensions, evidence list, validity. Copy `packages/catalog/models/agy/gemini-flash.yaml` to `~/.orchestra/catalog-overrides/models/agy/gemini-flash.yaml`, set `validTo: 2026-01-01` → within 2 s the row shows a *deprecated* badge and *local* source.
...
5. Delete the override file → Models badge disappears; repeat step 4 preview → chosen `agy/gemini-flash` with reason `policy-primary (default)`. Cancel without running.
...
9. **Resilience**: while a Lead `ask_user` is open, `kill -9` the daemon, restart it → the question is still in Attention; answer it → Lead receives the answer (bridge reconnects or Lead retries the tool). Board and Chat reconnect without reload.
