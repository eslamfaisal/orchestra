...
3. **Implement.** Delegate to the routed agent (`orchestra-architect` for tier A, `orchestra-implementer` for tier B, `orchestra-builder` for tier C) with: the step file path, the branch name `<step-id-lowercase>-<slug>` (create it from `main` first), and the instruction to work through §5 Tasks in order with one commit per task. If the step is tier B/C and lists sub-tasks that are pure boilerplate (catalog YAML, i18n strings, test scaffolds, docs), split those out to `orchestra-builder` in parallel when they touch different files.
...
5. **Review.** Delegate to `orchestra-reviewer` with the step file and the branch diff range. On `REQUEST CHANGES`, send blockers/majors back to the implementing agent, then re-review. Loop at most twice; after that, report to me.
